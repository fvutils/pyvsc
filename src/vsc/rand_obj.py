...
from vsc.coverage import covergroup
...
def randobj(T):
    ...
    if not hasattr(T, "_ro_init"):
        ...
        def randomize(self, cg=None, debug=0):
            model = self.get_model()
            cg_model = None
            if cg is not None:
                if not hasattr(cg, "_cg_int"):
                    raise Exception("parameter \"" + str(cg) + 
                                    "\" must be of covergroup type")
                    
                # Need to ensure that the covergroup matches the class
                cg_i = cg._get_int()
                if len(cg_i.sample_var_l) != 1:
                    raise Exception("Target covergroup may only have a single target, not " +
                                    str(len(cg_i.sample_var_l)))
                obj = cg_i.sample_obj_l[0]
                
                # This class must be of the same type or 
                # derived from the sample-argument class.
                if not issubclass(type(obj), type(self)):
                    raise Exception(""
                        "Randomized object " + str(self) + " must be of the same type "
                        " or a subclass of the object sampled for coverage " + str(obj))

                cg_model = cg.get_model()
                
            try:
                Randomizer.do_randomize([model], cg=cg_model, debug=debug)
            except SolveFailure as e:
                print(e.diagnostics)
                raise e
            
            if cg is not None:
                # If a covergroup was provided, sample it to record
                # progress
                cg.sample(self)
        ...
        def __exit__(self, t, v, tb):
            ...
            if self.cg is not None:
                if not hasattr(self.cg, "_cg_int"):
                    raise Exception("parameter \"" + str(self.cg) + 
                                    "\" must be of covergroup type")
                    
                # Need to ensure that the covergroup matches the class
                cg_i = self.cg._get_int()
                if len(cg_i.sample_var_l) != 1:
                    raise Exception("Target covergroup may only have a single target, not " +
                                    str(len(cg_i.sample_var_l)))
                obj = cg_i.sample_obj_l[0]
                ...
                cg_model = self.cg.get_model()
                
            try:
                Randomizer.do_randomize([model], [c], cg=cg_model, debug=self.debug)
            except SolveFailure as e:
                print(e.diagnostics)
                raise e
        ...
        def randomize_with(self, cg=None, debug=0, solve_fail_debug=False):
